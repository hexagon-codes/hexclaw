...
// Register 注册 Agent
func (r *Router) Register(cfg AgentConfig) error {
	if cfg.Name == "" {
...
func (r *Router) RemoveRules(agentName string) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
//  4. 平台匹配（Platform 匹配）
//  5. 默认 Agent
func (r *Router) Route(req RouteRequest) *RoutingResult {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
//   - 不匹配: -1
func (r *Router) matchScore(rule *Rule, req RouteRequest) int {
	score := 0
	matched := false
...
func (r *Router) ListAgents() []AgentConfig {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *Router) GetAgent(name string) (*AgentConfig, bool) {
	r.mu.RLock()
	defer r.mu.RUnlock()
